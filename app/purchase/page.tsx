    ...
    <main className="flex min-h-screen items-center justify-center bg-slate-950 px-4 py-20 text-slate-50">
      <div className="w-full max-w-5xl space-y-10">
        <header className="space-y-3 text-center">
          <span className="inline-flex items-center gap-2 rounded-full border border-sky-500/40 bg-sky-500/10 px-3 py-1 text-xs font-semibold text-sky-300">
            Satın Alma
          </span>
          <h1 className="text-3xl font-semibold tracking-tight text-sky-200">
        ...
        <section className="rounded-2xl border border-slate-800 bg-slate-900/70 px-6 py-5 text-sm text-slate-200">
          <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.3em] text-sky-300">Giriş yapan kullanıcı</p>
              <p className="text-base font-semibold text-slate-50">{mockUser.name}</p>
              <p className="text-xs text-slate-400">{mockUser.email}</p>
            </div>
            <div className="rounded-xl border border-sky-500/30 bg-sky-500/10 px-4 py-2 text-xs text-sky-200">
          ...
          {purchasePlans.map((plan) => (
            ...
            >
              <div>
                ...
                <p className="text-sm text-sky-300">{plan.price}</p>
              ...
              {plan.contact ? (
                <a
                  href={plan.href}
                  className="mt-auto inline-flex items-center justify-center rounded-full border border-sky-400 px-3 py-1.5 text-xs font-medium text-sky-100 transition hover:bg-sky-400 hover:text-sky-900"
                >
